...
def main():
    ...
    process = multiprocessing.Process(target=basic_test_and, args=(
        0.5, 10000, 1, 1, [0.557124099495, 0.818646852568, 0.780843895666, 0.754360037521, 0.59640163798]))
    jobs.append(process)

    process = multiprocessing.Process(target=basic_test_nand, args=(
        0.5, 10000, 1, 1, [0.638960808538, 0.969402879667, 0.126659914775, 0.650902226571, 0.868840437027]))
    jobs.append(process)

    process = multiprocessing.Process(target=basic_test_or, args=(
        0.5, 10000, 1, 1, [0.986729999424, 0.860659344576, 0.321420402441, 0.936216680739, 0.147877842286]))
    jobs.append(process)

    process = multiprocessing.Process(target=basic_test_nor, args=(
        0.5, 10000, 1, 1, [0.837878552368, 0.13364966718, 0.230012125002, 0.014312746897, 0.532260514866]))
    jobs.append(process)
    ...
    print '\nComplex tests:'
    process = multiprocessing.Process(target=xor_test, args=(0.5, 10000, 2, 8, xor_weights))
    jobs.append(process)

    process = multiprocessing.Process(target=x_squared_test, args=(0.2, 1000, 1, 8, x_squared_weights))
    jobs.append(process)
